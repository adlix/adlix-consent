...
export default function RegisterForm({ error: initialError, plan }: RegisterFormProps) {
  const router = useRouter()
  const [error, setError] = useState(() => {
    if (initialError === 'abgelehnt') return 'Registrierung abgelehnt.'
    if (initialError === 'social_login_failed')
      return 'Social-Registrierung fehlgeschlagen. Bitte erneut versuchen.'
    return ''
  })
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Bei plan=pro direkt zum Checkout weiterleiten
      if (plan === 'pro') {
        ...
        <div className="bg-white rounded-xl shadow-sm p-8">
          ...
          {plan === 'pro' && (
            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-700">
              Nach der Registrierung wirst du direkt zum Pro-Checkout weitergeleitet.
            </div>
          )}
          ...
          <div className="space-y-3">
            ...
            >
              ...
              Mit Telegram anmelden / registrieren
            </button>
          </div>
