...
const DEMO_PROJECTS = [
  {
    ...
    description: 'Dieses Vorhaben wurde bereits beschlossen. Es dient als Beispiel für den beschlossenen Status.',
  ...
  },
  {
    ...
    description: 'Dieses Vorhaben hat eine offene Abstimmungsrunde mit gemischten Votes. Es ist bereit für den Consent-Prozess.',
  ...
  },
  {
    ...
    description: 'Dieses Vorhaben ist noch im Entwurf. Es zeigt den draft-Status des Consent-Flows.',
...
const VOTES_FOR_ROUND = [
  { userEmail: 'demo1@example.com', choice: 'consent', reason: 'Klingt gut, keine Einwände.' },
  { userEmail: 'demo2@example.com', choice: 'minor_objection', reason: 'Ich hätte gerne mehr Details zur Umsetzung.' },
  ...
  try {
    ...
    const circleResult = await client.query(
      `INSERT INTO circles (name, description, invite_token, created_by, updated_by)
       VALUES ($1, $2, $3, $4, $4)
       RETURNING id`,
      ['Demo-Kreis', 'Ein Demo-Kreis zum Testen des Consent-Workflows.', 'demo-circle-token-2026', ownerId]
    )
    ...
    for (const user of userIds) {
      await client.query(
        `INSERT INTO circle_members (circle_id, user_id, role, created_by, updated_by)
         VALUES ($1, $2, $3, $1, $1)`,
        [circleId, user.id, user.email === 'demo1@example.com' ? 'admin' : 'member']
      )
    }
    ...
    for (const projectData of DEMO_PROJECTS) {
      ...
      console.log(`  Created project: ${projectData.name} (ID: ${result.rows[0].id})`)
    }
