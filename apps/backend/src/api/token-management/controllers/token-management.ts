...
export default ({ strapi }: { strapi: Strapi }) => ({
  ...
  async create(ctx) {
    ...
    if (!scope || !["read", "write", "admin"].includes(scope))
      ctx.throw(400, "scope must be read, write, or admin");

    const tokenValue = crypto.randomBytes(32).toString("hex");

    const tokenRecord = await strapi
      .documents("api::api-token.api-token")
      .create({
        data: {
          token: tokenValue,
          name,
