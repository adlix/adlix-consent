    ...
    if (roundId) {
      ...
      if (round.status === "information" && type !== "question") {
        return ctx.badRequest(
          "In der Informationsrunde sind nur Fragen erlaubt. Keine Meinungen oder Diskussion.",
        );
      }
      ...
      if (
        ...
        return ctx.badRequest(
          "In der Reaktionsrunde sind nur Perspektiven erlaubt. Kein Gegenargumentieren.",
        );
      }
      ...
      if (round.status === "voting" || round.status === "integration") {
        return ctx.badRequest(
          `In der Phase "${round.status}" sind keine Kommentare möglich.`,
        );
      }
    }
