...
module.exports = createCoreController("api::circle.circle", ({ strapi }) => ({
  ...
  async join(ctx) {
    ...
    if (existingMember) {
      return ctx.send({
        message: "Du bist bereits Mitglied dieses Kreises",
        circleId: circle.id,
      });
    }
  ...
  async generateInvite(ctx) {
    ...
    const circle = await strapi.entityService.findOne(
      "api::circle.circle",
      id,
      {
        populate: { owner: true },
    ...
    if (circle.owner?.id !== userId) {
      return ctx.forbidden(
        "Nur der Kreis-Ersteller kann Einladungen erstellen",
      );
    }
    ...
    return ctx.send({
      ...
      inviteUrl: `/circles/join/${newToken}`,
    });
  },
  ...
  async members(ctx) {
    ...
    const isMember = circleMembers.some(
      (m: { user?: { id: number } }) => m.user?.id === userId,
    );
    const isOwner = circle.owner?.id === userId;

    if (!isMember && !isOwner) {
      return ctx.forbidden(
        "Kein Zugriff — du bist kein Mitglied dieses Kreises",
      );
    }
