...
module.exports = createCoreController("api::dialog.dialog", ({ strapi }) => ({
  ...
  async advance(ctx) {
    ...
    if (dialog.status !== "active")
      return ctx.badRequest("Dialog ist nicht aktiv.");

    const currentPhaseNum = dialog.currentPhase;
    if (currentPhaseNum >= 6)
      return ctx.badRequest("Dialog ist bereits in der letzten Phase.");

    // Complete the current phase
    const currentPhase = (
      dialog.phases as { phaseNumber: number; documentId: string }[]
    ).find((p) => p.phaseNumber === currentPhaseNum);
    if (currentPhase) {
  ...
  async complete(ctx) {
    ...
    if (!["completed", "escalated"].includes(status)) {
      return ctx.badRequest(
        "Ungültiger Status. Erlaubt: completed, escalated.",
      );
    }
